function [] = draw_interpolation_figures( config, net_cpu, syn_Z, interp_type, epoch )
net = vl_simplenn_move(net_cpu, 'gpu');
% syn_Z: n_syn * config.z_dim
n_syn = size(syn_Z, 1);
Z = zeros([config.z_sz, n_syn], 'single');
for i = 1:n_syn
   Z(:,:,:,i) = syn_Z(i, :); 
end

fz_interp = vl_gan(net, gpuArray(Z), [], [], ...
        'accumulate', false, ...
        'disableDropout', true, ...
        'conserveMemory', 1 ...
        );
...
imwrite(I_syn,[config.Synfolder,  interp_type, '_interpolation_epoch_', num2str(epoch) ,'.png']);
end
