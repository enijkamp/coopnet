function imdb = cnn_imagenet_setup_data_hae(varargin)
...
if isempty(opts.subCat)
    ...
    [~, ori_labs] = ismember(opts.subCat, cats);
    lab = 1;
    for d = 1:length(opts.subCat)
        %         [~, lab] = ismember(d.name, cats);
        ims = dir(fullfile(opts.dataDir, 'images', 'train', opts.subCat{d}, '*.JPEG')) ;
        names{end+1} = strcat([opts.subCat{d}, filesep], {ims.name}) ;
        labels{end+1} = ones(1, numel(ims)) * lab ;
        fprintf('.') ;
        if mod(numel(names), 50) == 0, fprintf('\n') ; end
        lab = lab + 1;
...
if numel(ims) ~= 50e3
  ...
  labels =[] ;
    ...
    if isempty(opts.subCat)
        ...
        [keep, ~] = ismember(labels, ori_labs);
        keep = find(keep ~= 0);
        if ~isempty(valBlacklistPath)
            black = textread(valBlacklistPath, '%d') ;
            fprintf('blacklisting %d validation images\n', numel(black)) ;
            keep = setdiff(keep, black) ;
        end
        names = names(keep) ;
        labels = labels(keep) ;
        [~, labels] = ismember(labels, ori_labs);
    end
end
